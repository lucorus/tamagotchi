...
def feed_pet(cursor: sqlite3.Cursor, user_id: int):
...
def add_mood(cursor: sqlite3.Cursor, user_id: int, mood: int = 5) -> None:
    pet = get_pet_info(cursor, user_id)
    cursor.execute('''
        UPDATE pet SET mood=mood+? WHERE uuid=?;
        ''', (mood, pet[2])
    )
    cursor.execute('''
        UPDATE pet SET mood=max_mood WHERE uuid=? AND mood>?
    ''', (pet[2], pet[10]))
    cursor.connection.commit()
...
def buy_food(cursor: sqlite3.Cursor, user_id: int):
    cursor.execute(
        '''
        UPDATE users SET food = food + 1, coins = coins - 1 WHERE id=?
        ''', (user_id, )
    )
    cursor.connection.commit()
...
def create_pet(cursor: sqlite3.Cursor, user_id: int, name: str) -> None:
    ...
    cursor.execute(
        '''
        INSERT INTO pet (uuid, name) VALUES(?, ?);
        ''',
        (pet_uuid, name)
    )
...
def change_count_food_and_mood(cursor: sqlite3.Cursor) -> None:
    cursor.execute('''
                    UPDATE pet SET food=food - ?, mood=mood - ?
                    ''', (config.food_consumption, config.mood_consumption))
    cursor.connection.commit()
